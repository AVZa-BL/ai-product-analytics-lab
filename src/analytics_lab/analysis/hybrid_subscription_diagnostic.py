...
def engagement_summary(pairs: pd.DataFrame) -> dict[str, float | int | None]:
    ...
    return {
        ...
        "control_session_count_change": float(
            validated["control_session_count_change"].mean()
        ),
...
def _prior_payer_pairs(pairs: pd.DataFrame) -> pd.DataFrame:
    ...
    if prior_payers.empty:
        raise ValueError("revenue summary requires matched prior-payer pairs")
    return prior_payers


def revenue_summary(pairs: pd.DataFrame) -> dict[str, float | int]:
    ...
    return {
        ...
        "subscriber_total_revenue_change": float(
            mean["subscriber_total_net_revenue_usd_change"]
        ),
        "non_subscriber_total_revenue_change": float(
            mean["control_total_net_revenue_usd_change"]
        ),
...
def _cannibalization_aggregates(pairs: pd.DataFrame) -> pd.DataFrame:
    grouped = pairs.groupby(
        "subscriber_prior_payer_status", as_index=False, dropna=False
    ).agg(
        matched_pair_count=("pair_id", "size"),
        matched_subscriber_count=("subscriber_player_id", "nunique"),
        matched_control_count=("control_player_id", "nunique"),
...
def _reconcile_aggregate(
    ...
    *,
    ...
    published_ordered = published.loc[:, [*keys, *values]].sort_values(
        list(keys)
    ).reset_index(drop=True)
    if not expected_ordered.loc[:, keys].equals(published_ordered.loc[:, keys]):
        raise ValueError(f"{label} controls missing matched population")
    try:
        published_values = published_ordered.loc[:, values].apply(
            pd.to_numeric, errors="raise"
        )
    except (TypeError, ValueError) as error:
