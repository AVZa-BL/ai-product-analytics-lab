...
where cohort.as_of_at_utc is distinct from observation.as_of_at_utc
   or (cohort.cohort_type = 'eligible_exposure' and (
       cohort.is_conversion_mature is distinct from coalesce(
           cohort.cohort_latest_at_utc + interval '28 days' <= observation.as_of_at_utc, false)
       or (cohort.cohort_latest_at_utc + interval '28 days' > observation.as_of_at_utc
   ...
   or (cohort.cohort_type = 'subscription_start' and (
       cohort.is_d30_mature is distinct from coalesce(
           cohort.cohort_latest_at_utc + interval '30 days' <= observation.as_of_at_utc, false)
       or (cohort.cohort_latest_at_utc + interval '30 days' > observation.as_of_at_utc
