...
daily_events as (
    ...
    union all
    select f.player_id, cast(timezone('UTC', f.participated_at_utc) as date) as metric_date from {{ ref('stg_hybrid_subscription__live_event_participation') }} f
    union all
    select f.player_id, cast(timezone('UTC', f.transaction_at_utc) as date) as metric_date from {{ ref('fct_hybrid_subscription__store_transactions') }} f
    union all
    select f.player_id, cast(timezone('UTC', f.entitlement_start_at_utc) as date) as metric_date from {{ ref('fct_hybrid_subscription__subscription_entitlements') }} f
    union all
    select f.player_id, cast(timezone('UTC', f.exposed_at_utc) as date) as metric_date from {{ ref('fct_hybrid_subscription__marketing_exposures') }} f
    union all
    select f.player_id, cast(timezone('UTC', f.occurred_at_utc) as date) as metric_date from {{ ref('fct_hybrid_subscription__currency_grants') }} f
...
daily_components as (
    select k.*,
        (select count(*) from {{ ref('fct_hybrid_subscription__sessions') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.started_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and true) as session_count,
        (select coalesce(sum(f.duration_seconds),0) from {{ ref('fct_hybrid_subscription__sessions') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.started_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and true) as session_duration_seconds,
        (select count(*) from {{ ref('stg_hybrid_subscription__live_event_participation') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.participated_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and true) as liveops_participation_count,
        (select coalesce(sum(f.recognized_net_revenue_usd),0) from {{ ref('fct_hybrid_subscription__store_transactions') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.transaction_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and f.is_standalone_store_revenue) as standalone_store_net_revenue_usd,
        (select coalesce(sum(f.recognized_net_revenue_usd),0) from {{ ref('fct_hybrid_subscription__store_transactions') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.transaction_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and f.is_subscription_revenue) as subscription_net_revenue_usd,
        (select coalesce(sum(f.discount_amount_usd),0) from {{ ref('fct_hybrid_subscription__store_transactions') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.transaction_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and true) as discount_amount_usd,
        (select count(*) from {{ ref('fct_hybrid_subscription__subscription_entitlements') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.entitlement_start_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and true) as new_subscription_count,
        (select count(*) from {{ ref('fct_hybrid_subscription__marketing_exposures') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.exposed_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and f.is_incrementality_eligible) as eligible_exposure_count,
        (select count(*) from {{ ref('fct_hybrid_subscription__marketing_exposures') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.exposed_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and not f.is_incrementality_eligible) as rejected_exposure_count,
        (select count(*) from {{ ref('fct_hybrid_subscription__currency_grants') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.occurred_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and f.entry_type = 'subscription_grant') as subscription_grant_count,
        (select count(*) from {{ ref('fct_hybrid_subscription__currency_grants') }} f join {{ ref('dim_hybrid_subscription__players') }} p using (player_id) where cast(timezone('UTC', f.occurred_at_utc) as date)=k.metric_date and p.prior_payer_status=k.prior_payer_status and f.entry_type = 'subscription_grant' and f.is_reconciled) as reconciled_subscription_grant_count,
...
cohort_members as (
    select 'eligible_exposure' as cohort_type,x.player_id,x.index_at as cohort_at,
        ...
        case when s.start_at > x.index_at and s.start_at < x.index_at + interval '672 hours'
    ...
    select 'subscription_start',s.player_id,s.start_at,0,0,1,
        ...
        case when s.start_at + interval '720 hours' <= b.as_of_at_utc and exists (
...
cohort_components as (
    select cohort_type,cast(timezone('UTC', cohort_at) as date) as cohort_date,
        ...
        cohort_type='eligible_exposure' and max(cohort_at)+interval '672 hours'<=b.as_of_at_utc as is_conversion_mature,
        cohort_type='subscription_start' and max(cohort_at)+interval '720 hours'<=b.as_of_at_utc as is_d30_mature
    from cohort_members cross join bounds b
    group by cohort_type,cast(timezone('UTC', cohort_at) as date),b.as_of_at_utc
),
expected_cohorts as (
    select *,
...
monthly_missing as (select metric_month,as_of_at_utc,is_month_complete,mau,month_start_active_subscriber_count,month_end_active_subscriber_count,expired_or_revoked_entitlement_count,standalone_store_net_revenue_usd,subscription_net_revenue_usd,eligible_standalone_transaction_count,discounted_standalone_transaction_count,liveops_participant_count,total_net_revenue_usd,arpmau,subscriber_churn_rate,discount_utilization_rate,liveops_participation_rate from expected_monthly except all select metric_month,as_of_at_utc,is_month_complete,mau,month_start_active_subscriber_count,month_end_active_subscriber_count,expired_or_revoked_entitlement_count,standalone_store_net_revenue_usd,subscription_net_revenue_usd,eligible_standalone_transaction_count,discounted_standalone_transaction_count,liveops_participant_count,total_net_revenue_usd,arpmau,subscriber_churn_rate,discount_utilization_rate,liveops_participation_rate from {{ ref('mart_hybrid_subscription__monthly_kpis') }}),
monthly_unexpected as (select metric_month,as_of_at_utc,is_month_complete,mau,month_start_active_subscriber_count,month_end_active_subscriber_count,expired_or_revoked_entitlement_count,standalone_store_net_revenue_usd,subscription_net_revenue_usd,eligible_standalone_transaction_count,discounted_standalone_transaction_count,liveops_participant_count,total_net_revenue_usd,arpmau,subscriber_churn_rate,discount_utilization_rate,liveops_participation_rate from {{ ref('mart_hybrid_subscription__monthly_kpis') }} except all select metric_month,as_of_at_utc,is_month_complete,mau,month_start_active_subscriber_count,month_end_active_subscriber_count,expired_or_revoked_entitlement_count,standalone_store_net_revenue_usd,subscription_net_revenue_usd,eligible_standalone_transaction_count,discounted_standalone_transaction_count,liveops_participant_count,total_net_revenue_usd,arpmau,subscriber_churn_rate,discount_utilization_rate,liveops_participation_rate from expected_monthly),
