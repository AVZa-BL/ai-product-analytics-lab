...
where is_population_eligible
  and (
    ...
    or regexp_matches(player_id, '^[[:space:]]*$')
    or prior_payer_status is null
    or regexp_matches(prior_payer_status, '^[[:space:]]*$')
    or prior_payer_status not in ('prior_payer', 'prior_nonpayer')
    or platform is null
    or regexp_matches(platform, '^[[:space:]]*$')
    or acquisition_channel is null
    or regexp_matches(acquisition_channel, '^[[:space:]]*$')
