...
conversion as (
    select
        ...
        cast(e.first_exposure_at_utc as date) as cohort_date,
        ...
        count(distinct case when s.first_start_at_utc > e.first_exposure_at_utc
            and s.first_start_at_utc < e.first_exposure_at_utc + interval '28 days'
        ...
        max(e.first_exposure_at_utc) + interval '28 days' <= o.as_of_at_utc as is_conversion_mature,
    ...
    group by cast(e.first_exposure_at_utc as date), o.as_of_at_utc
),
retention as (
    select
        'subscription_start' as cohort_type,
        cast(s.first_start_at_utc as date) as cohort_date,
        ...
        count(distinct case when s.first_start_at_utc + interval '30 days' <= o.as_of_at_utc
        ...
        max(s.first_start_at_utc) + interval '30 days' <= o.as_of_at_utc as is_d30_mature
    ...
    left join {{ ref('fct_hybrid_subscription__subscription_entitlements') }} e
        ...
        and e.entitlement_start_at_utc <= s.first_start_at_utc + interval '30 days'
        and e.entitlement_end_at_utc > s.first_start_at_utc + interval '30 days'
    where s.first_start_at_utc <= o.as_of_at_utc
    group by cast(s.first_start_at_utc as date), o.as_of_at_utc
),
cohorts as (
